...
Цель модуля — построить модель, выделяющую безопасные и опасные классы поверхности для дальнейшего использования в `Pipeline`.
...
- выполняет рандомизацию среды (время суток, туман/пыль);
- телепортирует дрон в точки сетки со случайным отклонением;
